	...
	private static func loadDigests(from provider: NSItemProvider) async -> [FurnitureDigest] {
		...
		if collected.isEmpty {
			...
			for typeIdentifier in importableTypeIdentifiers {
				...
				collected.append(
					contentsOf: InventoryTransfer.digests(fromData: data, suggestedName: provider.suggestedName))
			}
		}
...
extension NSItemProvider {
	fileprivate func loadBestEffortData(forTypeIdentifier typeIdentifier: String) async throws -> Data {
	...
	fileprivate func loadDataRepresentationAsync(forTypeIdentifier typeIdentifier: String) async throws -> Data? {
	...
	fileprivate func loadFileRepresentationData(forTypeIdentifier typeIdentifier: String) async throws -> Data {
		try await withCheckedThrowingContinuation { continuation in
	...
	fileprivate func loadBestEffortFileURL() async throws -> URL? {
