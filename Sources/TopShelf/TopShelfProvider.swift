...
final class TopShelfProvider: TVTopShelfContentProvider {
    ...
    override init() {
        ...
        logger.info("Top Shelf provider initialized. bundleID=\(Bundle.main.bundleIdentifier ?? "nil", privacy: .public)")
    }
    ...
    private static func itemIdentifier(for snapshot: InventorySnapshot) -> String {
        ...
        return "inventory.summary.\(snapshot.furnitureCount).\(snapshot.distinctThingCount).\(snapshot.totalThingCount).\(timestamp)"
    }

    private static func imageFileName(for snapshot: InventorySnapshot) -> String {
        let timestamp = Int(snapshot.generatedAt.timeIntervalSince1970 * 1000)
        return "top_shelf_\(snapshot.furnitureCount)_\(snapshot.distinctThingCount)_\(snapshot.totalThingCount)_\(timestamp).png"
    }
