...
struct FurnitureTreeListView: View {
    ...
    private var nodes: [InventoryTreeNode] {
        digests.map { furniture in
            let children = furniture.things.map { thing in
                InventoryTreeNode(
                    id: "\(InventoryMath.normalizedLookupKey(furniture.name))::\(InventoryMath.normalizedLookupKey(thing.name))",
                    title: thing.name,
    ...
    var body: some View {
            ...
            .accessibilityIdentifier("furniture_tree_list")
                ...
                List {
                    ForEach(nodes) { node in
                        ...
                        #else
                            Button {} label: { furnitureNodeContent(node) }
                                .buttonStyle(.plain)
                        #endif
                    }
                }
        ...
        private func rowActions(digest: FurnitureDigest, name: String) -> some View {
            Button(role: .destructive) { onDelete(digest) } label: {
