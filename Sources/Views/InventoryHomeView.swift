...
struct InventoryHomeView: View {
    ...
    let iCloudAvailable: Bool
        ...
        private func applyNavigation(_ request: InventoryNavigation.Request) {
            ...
            let id = request.section == .furniture
    ...
    private var toolbarGroups: [[ToolbarAction]] {
        let sections: [ToolbarAction] = InventorySection.allCases.map { section in
            ToolbarAction(
                ...
                canOverflow: false
            ) { selectedSection = section; selection = [] }
        }
            ...
            if let xmlExportURL, let zipExportURL {
                commands.append(
                    ToolbarAction(
                        ...
                        shareItems: [
                            ToolbarShareItem(title: String(localized: "XML file"), systemImage: "doc.text", url: xmlExportURL),
                            ToolbarShareItem(title: String(localized: "ZIP archive"), systemImage: "doc.zipper", url: zipExportURL),
                        ]
                    ) {}
                )
            }
    ...
    var body: some View {
        ...
        .onReceive(NotificationCenter.default.publisher(for: .persistentStoreRemoteChange).receive(on: RunLoop.main)) { _ in
            InventorySnapshotPublisher.publish(context: modelContext)
        }
        #if os(tvOS)
        .onReceive(NotificationCenter.default.publisher(for: UIApplication.willResignActiveNotification).receive(on: RunLoop.main)) { _ in
            InventorySnapshotPublisher.publishTopShelf(snapshot: snapshot)
        }
        #endif
        #if !os(macOS)
